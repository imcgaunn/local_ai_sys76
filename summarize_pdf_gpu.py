#!/usr/bin/env python3
"""Summarize a long PDF locally with Qwen2.5-7B-Instruct fully offloaded to
the RTX 5070 via llama-server (CUDA build).

Same map-reduce pipeline as summarize_pdf.py:
...
Differences vs summarize_pdf.py:
  - Points at the CUDA llama.cpp build (build-cuda/) and Qwen2.5-7B Q4_K_M
  - Spawns llama-server with -ngl 99 -fa for full GPU offload + flash attn
  - Different default port (8766) so a CPU-Gemma server can run alongside

Usage:
    ./summarize_pdf_gpu_setup.sh                            # one-time
    .venv/bin/python summarize_pdf_gpu.py path/to/doc.pdf
    .venv/bin/python summarize_pdf_gpu.py doc.pdf -o summary.md
    .venv/bin/python summarize_pdf_gpu.py doc.pdf --server-url http://127.0.0.1:8766
"""
...
DEFAULT_LLAMA_SERVER = REPO / "llama.cpp" / "build-cuda" / "bin" / "llama-server"
DEFAULT_GGUF = REPO / "models" / "Qwen2.5-7B-Instruct-Q4_K_M.gguf"
DEFAULT_PORT = 8766

# Qwen2.5's tokenizer averages ~3.5 chars/token for English, same as Gemma.
CHARS_PER_TOKEN = 3.5
...
def extract_pdf_text(pdf_path: Path) -> str:
    try:
        ...
        try:
            ...
            sys.exit("pymupdf not installed. Run ./summarize_pdf_gpu_setup.sh")
...
def map_step(url: str, chunks: list[str], summary_tokens: int) -> list[str]:
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        if not s.strip():
            ...
            print("     Inspect the server log (summarize_pdf_gpu.llama-server.log)")
...
def main() -> None:
    ...
    log_path = REPO / "summarize_pdf_gpu.llama-server.log"
